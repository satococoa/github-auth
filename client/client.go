...
func CreateClient(appName string) *github.Client {
	// 設定ファイルがあれば読み込む
	var accessToken string
	var err error

	user, _ := user.Current()
	conf := user.HomeDir + "/." + strings.ToLower(appName) + ".conf"
	buf, err := ioutil.ReadFile(conf)
	if err != nil {
		// 認証して accessToken を取得する
		accessToken, err = fetchAccessToken(appName)
		if err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
...
func fetchAccessToken(appName string) (string, error) {
	scan := func() string {
	...
	fmt.Printf("\t Errors: %v\n", e.Message)
	return "", errors.New("既に " + appName + " という Personal API Token が存在する場合は削除してください。")
}
